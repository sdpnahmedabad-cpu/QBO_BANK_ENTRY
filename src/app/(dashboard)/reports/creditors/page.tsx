...
function EmailDialog({ open, onClose, vendorName, totalAmount, companyName }: EmailDialogProps) {
    ...
    // Fetch vendor email from QBO and set defaults when dialog opens
    useEffect(() => {
        if (!open) {
            setStatus("idle");
            setErrorMsg("");
        ...
        setSubject(`Payment Confirmation – ${companyName}`);
        setBody(
            `Dear ${vendorName},\n\n` +
            `This is regarding our outstanding payable of ${totalAmount} with your company.\n\n` +
            `We would like to confirm the outstanding balance and discuss the payment schedule.\n\n` +
            `Please share the updated statement of account at your earliest convenience.\n\n` +
            `Thank you for your continued partnership.\n\n` +
            `Best regards,\n${companyName}`
        );

        // Fetch email from QBO (vendor endpoint)
        setFetchingEmail(true);
        setTo("");
        fetch(`/api/qbo/vendor-email?name=${encodeURIComponent(vendorName)}`)
            .then(res => res.json())
            .then(data => {
                setTo(data.email || "");
            })
    ...
    }, [open, vendorName, totalAmount, companyName]);
            ...
            <div className="relative w-full max-w-lg mx-4 bg-card border border-white/10 rounded-xl shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-foreground">Send Email to Vendor</h3>
...
function CreditorsContent() {
    ...
    const [selectedVendor, setSelectedVendor] = useState({ name: "", total: "" });

    const fetchReport = (selectedDate: Date) => {
        setLoading(true);
        const dateStr = selectedDate.toISOString().split('T')[0];
        const url = `/api/qbo/reports/aged-payables?date=${dateStr}`;
        console.log(`[Client] Fetching creditors report from: ${url}`);
    ...
    const handleSendEmail = (vendorName: string, totalAmount: string) => {
        setSelectedVendor({ name: vendorName, total: totalAmount });
    ...
    const handleExportPDF = () => {
        ...
        doc.text("Creditors Outstanding Report", pageWidth / 2, 25, { align: "center" });
        ...
        doc.save(`${companyName.replace(/ /g, "_")}_Creditors_Outstanding_${format(date, "yyyy-MM-dd")}.pdf`);
    };
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Creditors Outstanding");
        XLSX.writeFile(wb, `${companyName.replace(/ /g, "_")}_Creditors_Outstanding_${format(date, "yyyy-MM-dd")}.xlsx`);
    };
    ...
    return (
        <div className="space-y-6">
                    ...
                    <CardContent>
                        <div className="overflow-x-auto">
                                        ...
                                        return (
                                            <tr key={i} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-4 py-4 text-center">
                                                    {isDataRow && vendorName && (
                                                        <button
                                                            onClick={() => handleSendEmail(vendorName, totalValue)}
                                                            ...
                                                            title={`Send email to ${vendorName}`}
                                                        >
                            ...
                            {(!reportData.Rows.Row || reportData.Rows.Row.length === 0) && (
                                <p className="text-center p-8 text-muted-foreground">No open bills found.</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
            ...
            <EmailDialog
                ...
                totalAmount={selectedVendor.total}
